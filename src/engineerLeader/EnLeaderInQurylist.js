...
function EnLeaderInQurylist({ checkPermission }) {
    ...
    const list = async () => {//현재 목록 불러오기
        ...
        const response = await axios.get(`http://13.124.230.133:8888/api/main/csEngineerLeaderList?currentPage=${currentPage}&postsPerPage=${postsPerPage}&cs_writer=${checkPermission.sub}&role=${checkPermission.role}`)
