  ...
  const list = async () => {
    ...
    if (searchValue.length == 0) {
      const response = await axios.get(
        `/api/main/AnnoList?currentPage=${currentPage}&postsPerPage=${postsPerPage}&role=${checkPermission.role}`
      );
      ...
      //검색결과가 1이상이라면?
      const result = currentPosts.filter(
        (data) =>
          data.notice_target == checkPermission.role ||
          data.notice_target == "ALL"
      );
      console.log(result);
      setPageInfo(result);
    }
  };
  const totalListNum = async () => {
    //토탈 목록 불러오기
                        ...
                        {pageInfo &&
                          pageInfo.slice(0, -1).map((item, index) => (
                            <tr key={index}>
                              ...
                              <td>
                                {item.notice_target == "ROLE_ENGINEER"
                                  ? "엔지니어"
                                  : null}
                                {item.notice_target == "ALL"
                                  ? "전체 공개"
                                  : null}
                              </td>
                            </tr>
